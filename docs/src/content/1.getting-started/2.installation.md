...
Replace `<version>` with a published version from
[GitHub Releases](https://github.com/mubaidr/gem-team/releases).

APM stores the resolved commit in `apm.lock.yaml`. A later `apm install`
reinstalls that locked commit instead of silently selecting a newer one.
...
Use `apm update --check` only to check whether the **APM CLI** itself has an
update. It does not update installed packages.
...
Use `--refresh` when troubleshooting a stale upstream cache. Normal package
upgrades should use `apm update [PACKAGE] --yes`.
...
Direct Git installs read Gem Team's source primitives from `.apm/`. The
repository's `build/` directory is generated release output and is not required
for installation.

## Next

[Usage ->](/getting-started/usage) · [Workflow ->](/guide/workflow) · [Agents ->](/agents)
