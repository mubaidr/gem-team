...
Every `new_task` gets a new plan ID and fresh `plan.yaml` plus `context_envelope.json`; no prior plan artifacts or context caches are loaded automatically.
...
Note: Task results are returned as structured JSON by each specialist agent; they are not persisted as separate files.
...
Do not use fuzzy names or partial IDs. To use an existing plan for background information, explicitly say `reference plan <exact-plan-id>`; the current task still receives a new plan ID, and imported facts are revalidated and source-attributed.
