...
```yaml
orchestrator:
  max_concurrent_agents: 4 # increase parallelism
  git_commit_on_gate_pass: false
testing:
  screenshot_on_failure: false
```

### Strict Quality Example

```yaml
quality:
  visual_regression_enabled: true
  visual_diff_threshold: 0.99
  a11y_audit_level: full
planning:
  enable_critic_for: [MEDIUM, HIGH]
```
...
A fully commented default file is available at [`.gem-team.yaml`](https://github.com/mubaidr/gem-team/blob/v0.4.0/.gem-team.yaml).

## Next Steps

- [Agent Team →](/agents) — explore each specialist agent's role
- [Production Best Practices →](/resources/operational-notes) — tips for real-world usage
