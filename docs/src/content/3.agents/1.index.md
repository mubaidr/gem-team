...
| **Planner**     | Validates the baseline and builds a minimal plan with ordered waves, routing, handoffs, and risks. Persists `docs/plan/{plan_id}/plan.yaml`, applies YAGNI/KISS, slices along concern boundaries, coalesces tasks that share a context base, and runs a scope-reduction gate plus a mechanical self-check before scoping tasks.                                                      |
